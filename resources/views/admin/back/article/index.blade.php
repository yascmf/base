                    ...
                    <form action="{{ _route('admin:article.index') }}" method="get">
                        <div class="input-group">
                            <input type="text" class="form-control input-sm pull-right" name="s_title"
                                   value="{{ request('s_title') }}" style="width: 150px;" placeholder="搜索文章标题">
            ...
            <table class="table table-hover table-bordered">
                ...
                @foreach ($articles as $art)
                    <tr>
                        <td>
                            @can('article-write')
                                <a href="{{ _route('admin:article.edit', $art->id) }}">
                        ...
                        <td class="text-green">{{ tag_word($art->tags, $_tags) }}</td>
                        <td class="text-muted" title="{{ $art->title }}">{{ str_limit($art->title, 36) }}</td>
                        <td title="//example.org/{category}/{{ $art->slug }}.html"><span
                                    class="text-red">{{ str_limit($art->slug, 16) }}</span></td>
                        <td class="text-green">
                            @if(empty($art->thumb))
                                -
                            @else
                        ...
                        <td class="text-red"><a
                                    href="{{ _route('admin:article.index', ['s_cid' => $art->cid] ) }}">{{ $art->category->name }}</a>
                        </td>
