                ...
                <div class="tab-pane active" id="tab_1">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="title" autocomplete="off"
                               value="{{ old('title', isset($article) ? $article->title : null) }}" placeholder="标题">
                    </div>
                    <div class="form-group">
                        <label>网址缩略名(slug)
                            <small class="text-red">*</small>
                            <span class="text-green">[a-z0-9\-_]{1,120}</span>
                            <a href="javascript:void(0);" class="auto-to-pinyin">
                                <i class="fa fa-fw fa-hand-o-down" title="自动转换"></i></a></label>
                        <div class="input-group mono url_slug">
                            <p>//example.org/{category}/
                                <input type="text" id="slug" name="slug" autocomplete="off"
                                       value="{{ old('slug', isset($article) ? $article->slug : null) }}" class="slug"
                                       maxlength="120" pattern="[a-z0-9_-]{1,120}">.html
                            </p>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            @foreach(config('ecms.flag.articles') as $k => $v)
                                <input type="checkbox" name="flag[]" value="{{ $k }}"
                                        {{ (check_string($article->flag, $k) === true) ? 'checked' : '' }}>
                                <label class="choice" for="flag[]" title="{{ $v }}" data-value="{{ $k }}"
                                       style="cursor: pointer;">[{{ $k }}]{{ $v }}</label>
                            @endforeach
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>缩略图
                            <a href="javascript:void(0);" class="uploadPic" data-id="thumb">
                                <i class="fa fa-fw fa-picture-o" title="上传"></i></a>
                            <a href="javascript:void(0);" class="previewPic" data-id="thumb">
                                <i class="fa fa-fw fa-eye" title="预览小图"></i></a></label>
                        <input type="text" class="form-control" id="thumb" name="thumb"
                               value="{{ old('thumb', isset($article) ? $article->thumb : null) }}"
                               placeholder="缩略图地址：如{{ url('') }}/assets/img/yas_logo.png" readonly="readonly">
                    </div>
                    <div class="form-group">
                        <label>Tags
                            <small class="text-red">*</small>
                            <span class="text-green">最多可选4个</span></label>
                        <div class="input-group">
                            @foreach($tags as $k => $v)
                                <input type="checkbox" name="tag[]" value="{{ $v['id'] }}"
                                        {{ (check_string($article->tags, $v['id']) === true) ? 'checked' : '' }}>
                                <label class="choice" for="tag[]" title="{{ $v['name'] }}" data-value="{{ $v['id'] }}"
                                       style="cursor: pointer;">[{{ $v['id'] }}]{{ $v['name'] }}</label>
                            @endforeach
                        </div>
                    </div>
                    <div class="form-group">
                        <label>文章摘要
                            <small class="text-red">*</small>
                            <span class="text-green">min:10</span></label>
                        <textarea class="form-control" name="description" rows="3" cols="200" autocomplete="off"
                                  placeholder="文章摘要">{{ old('description', isset($article) ? $article->description : null) }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div id="mdeditor">
                          <textarea class="form-control" name="content" style="display:none;">
                              {{ old('content', isset($article) ? $article->content : '> hello editor.md') }}</textarea>
                        </div>
                    </div>
                </div><!-- /.tab-pane -->
...
@section('filledScript')
    ...
    $('label.choice').click(function() {
    var value = $(this).data('value');
    $('input[name="tag[]"][value='+value+']').iCheck('toggle');
    });
